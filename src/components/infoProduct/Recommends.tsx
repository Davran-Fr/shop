...
export const Recommends = ({ show }: { show: boolean }) => {
  ...
  const {containerRef, showMore , toggleAccordion , widht} = useReccommendAnimation(show)
  ...
  const showMoreBtn = clsx(
    'absolute w-full flex justify-center bg-showMoreGradient rounded-t-[50px] bottom-0  py-5 z-10',
    widht < 1280 && (data?.products.length !== undefined ? data.products.length : 5) >= 5  ? 'block' : 'hidden'
  )
    ...
    <div ref={containerRef} className={showDiv}>
      ...
      <div className={showMoreBtn}>
        <button
          ...
          className="bg-orderBtn px-10 py-2 font-world text-xl rounded-full"
        >
          Show More
        </button>
      </div>
